param(
    ...
    #[string]$configfile = "..\input\Example.xml"
)
...
function ReturnNewBVObject {
    ...
    $obj | Add-Member "ConfigurationNo" ""
    $obj | Add-Member "BBNo" ""              
      ...
      if ($value -eq "Configuration") {
        foreach ($p in $f.properties.GetEnumerator()) {
          switch ($AttributeHash[($p.name) -replace ".*\/",""]) {
            ...
            "ConfigurationNo"   { $obj.ConfigurationNo = $p.constant.'#text'; break}
...
function GenerateCfgConfigurationsXml ($file) {
  ...
  foreach ($c in $BVHash.Values.GetEnumerator() | Sort-Object -Property ConfigurationNo) {
    $output += "  <MTCConfiguration>`n"
    $output += "    <ConfigurationName>"+$c.ConfigurationName+"</ConfigurationName>`n"
    $output += "    <ConfigurationNo>"+$c.ConfigurationNo+"</ConfigurationNo>`n"
...
function GenerateReleaseBuildInclude () {

  $rep = ""
  $first = $true
  foreach ($bv in $BVHash.Values.GetEnumerator()) {
    if ($bv.ConfigurationType -eq "RELEASE") {
      if ($first) {
        $rep += "(RBFS_BUILDCONFIG  == RBFS_BUILDCONFIG_"+$bv.ConfigurationName+") `n"
        $first = $false
      } else {
        $rep += "#elif (RBFS_BUILDCONFIG  == RBFS_BUILDCONFIG_"+$bv.ConfigurationName+") `n"        
      }
      $rep += "  #include `"rbl/"+$GlobalTemplateHash["#cust#"]+"/"+$GlobalTemplateHash["#prj#"]+"_"+$GlobalTemplateHash["#prod#"]+"/cswpr/cfg/ReleaseBuild_"+$bv.ConfigurationName+"/RB_CSWPr_GenBFW.bcfg`"`n"
    }
  }
  if ($first) {
    $GlobalTemplateHash.Add("#ReleaseBuildInclude#","(0)")
  } else {
    $GlobalTemplateHash.Add("#ReleaseBuildInclude#",$rep)
  }
}
...
function GenerateBcfgFiles ($output, $template) {
  ...
  foreach ($bv in $BVHash.Values.GetEnumerator()) {
    if ($bv.ConfigurationType -eq "RELEASE") {
      ...
      $in = $template+"\RB_CSWPr_GenBFW.bcfg_tpl"
      $out = $output+"\rbl\"+$GlobalTemplateHash["#cust#"]+"\"+$GlobalTemplateHash["#prj#"]+"_"+$GlobalTemplateHash["#prod#"]+"\cswpr\cfg\ReleaseBuild_"+$LocalTemplateHash["#configname#"]+"\RB_CSWPr_GenBFW.bcfg"
...
function GenerateLoadrule ($output, $template) {
  ...
  $out = $output+"\rbl\"+$GlobalTemplateHash["#cust#"]+"\"+$GlobalTemplateHash["#prj#"]+"_"+$GlobalTemplateHash["#prod#"]+"\project\"+$GlobalTemplateHash["#prj#"]+"_"+$GlobalTemplateHash["#prod#"]+".loadrule"
  ExpandTemplate -in $in -out $out -localrep $null
}
...
function GenerateGlobalSwitchSettingHeader($outpath) {
  ...
  $out += " **********************************************************************************************************************`n"
  $out += " *`n"
  $out += " * COPYRIGHT RESERVED, Robert Bosch GmbH, 2019. All rights reserved.`n"
  $out += " * The reproduction, distribution and utilization of this document as well as the communication of its contents to`n"
  $out += " * others without explicit authorization is prohibited. Offenders will be held liable for the payment of damages.`n"
  $out += " * All rights reserved in the event of the grant of a patent, utility model or design.`n"
  $out += " *`n"
  $out += " **********************************************************************************************************************`n"
  $out += " */`n"
...
function GenerateGlobalHeader ($output) {
  ...
  $outpath = $output+"\rbl\"+$GlobalTemplateHash["#cust#"]+"\"+$GlobalTemplateHash["#prj#"]+"_"+$GlobalTemplateHash["#prod#"]+"\cswpr\cfg\"
...
function GenerateMTCStartScripts($output,$template){
  ...
  foreach ($bv in $BVHash.Values.GetEnumerator()) {
      ...
      $LocalTemplateHash.Add("#clean#","--clean ")
...
function Main(){
  ...
  GenerateMTCStartScripts -output $OutputFolder -template $TemplateFolder
