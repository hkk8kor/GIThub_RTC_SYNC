#ifndef RBSYS_FLASHOPTIONBYTES_D4_H__
#define RBSYS_FLASHOPTIONBYTES_D4_H__
 ...
 *
 * @file
 * @brief Flash Option Bytes defines for Renesas D4
 *
 * @copyright
...
RB_ASSERT_SWITCH_SETTINGS(RBFS_HSWMainOscillator,
                        RBFS_HSWMainOscillator_XTAL16MHz,
                        RBFS_HSWMainOscillator_XTAL20MHz);
...
// Offset 0x70
#define FAL_MODE          (1)     /* Only available on D1 and D2 devices, flag is reserved and must be 1 for all other devices */
#define OPAUDR            (1)

#if( RBFS_Ethernet == RBFS_Ethernet_Enabled )
  #define ETHDISABLE      (1)     /* enable Ethernet module */
#else
  #define ETHDISABLE      (0)     /* disable Ethernet module */
#endif
...
#if(RBFS_HSWMainOscillator == RBFS_HSWMainOscillator_XTAL20MHz)
    #define PLL0NDIV          (0x2Fu)
#else
...
// Offset 0xA8
#define PE2DIS            (0x00u)  /* PE2DIS (PE2 enable disable)        1: PE2 disabled           0: PE2 enabled */
#define RD_SEL            (0x04u)  /* RD_SEL */
#define PE2PB             (0x01u)  /* PE2PB (PE2 parity bit)             1: Parity of PE2 enabled  0: Parity of PE2 disabled */
#define EMF               (0x06u)  /* EMF (Emulation mode select flag) */

// Offset 0xAC
#if(RBFS_HSWMainOscillator == RBFS_HSWMainOscillator_XTAL20MHz)
    #define AMPSEL          (0x01u)  /* AMPSEL (MainOSC AMP config) */
#else
    #define AMPSEL          (0x02u)  /* AMPSEL (MainOSC AMP config) */
#endif
#define CAPSEL            (0x05u)  /* CAPSEL (MainOSC capacitance config) */
