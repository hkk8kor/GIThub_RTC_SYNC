...
#define OS_TARGET_PATCH_VERSION (11U)
...
#ifndef __cplusplus
  extern int main (int argc, char **argv, char **envp); /*lint -esym(752,main) allow to be unreferenced */
#endif
...
/* If the GHS header has already been included map directly on the intrinsic
 * function rather that line function as __memory_changed() causes a build error */
#ifdef __V800_IS
#define OS_STSR(regID, selID) __STSR((regID), (selID))
#else
static inline uint32 __attribute__((unused)) OS_STSR(sint32 regID, sint32 selID)
{
  uint32 ret;
  __memory_changed();
  ret = __STSR(regID, selID);
  __memory_changed();
  return ret;
}
#endif /* __V800_IS */
...
/* lint/MISRA raises warnings when using pointers to memory mapped registers
 so when lint checking pretend these are external variables. */
...
#include "Os_MemMap.h" /* [MISRA 2012 Dir 4.10] */ /*lint !e537 !e451 */
/*lint -restore */
extern const uint16 Os_CoreMapping[];
/* [MISRA 2012 Rule 20.1] */ /*lint -save -estring(9019, *) */
...
#define GetCoreID() ((CoreIdType)/*lint --e(9034) [MISRA 2012 Dir 10.3] Depends on CoreIdType size */Os_CoreMapping[OS_STSR(OS_PEID_REGID, OS_PEID_SELID)])  /* [$UKS 1619] [$UKS 1620] [$UKS 1621] */
...
#define Os_ErrorInformation (Os_const_coreconfiguration[Os_CoreMapping[OS_STSR(OS_PEID_REGID, OS_PEID_SELID)]].controlled->ErrorInformation)
